...
- [docs/design-v1.md](../design-v1.md) for Option 2B module boundaries, normalized contracts, safety rules, and command flows.
...
- Why not more abstract: V1 manages Skills only; the plan should preserve a V2 Subagent path through sibling item modules and typed fields, not a generic resource graph or platform roadmap.
- Next plausible change considered: later Configured Item kinds can be added beside Skills if the V1 contracts stay typed and item-specific.

## Milestones

### M1: Stable Contracts And Workspace Boundary

Establish the Rust workspace shape, shared result/error conventions, command request vocabulary, config and lockfile model boundaries, Manifest model, and typed normalized contracts for `ConfigRequest`, `PinnedConfig`, `LockfilePinnedConfig`, `PlannedPinnedConfig`, and `ObservedInstallation`.

Importance: contracts are load-bearing. Vague shapes will leak CLI wording, filesystem details, or persistence schema into modules that should exchange normalized contracts. This milestone is complete when data passed between modules has stable names, the CLI/core boundary is thin, and the codebase compiles and tests without real install behavior.
...
Importance: leaf modules are where ad hoc parsing and duplicated probing most easily creep in. `content_digest` especially needs deterministic tests because it affects resolution, content-addressed Managed Skill Content, artifact updates, and interrupted-write recovery. This milestone is complete when higher modules can rely on structured facts and narrow persistence APIs instead of re-probing filesystem state.

### M3: Resolution, Installation Observation, And Planning Policy

Implement the pure config, resolution, installation observation, and planning modules: `config`, `resolution`, `installation`, and `planning`. The planning module owns command-specific lifecycle policy for preview, apply, prune, and status while remaining independent of source resolution side effects, lockfile persistence, and filesystem mutation.

Importance: this is where config intent, repeatable pinning, observed evidence, and lifecycle policy become separate. The planning module should be one of the most heavily tested modules in V1 because bugs in blockers, stale state, prune skips, status findings, or unmanaged artifact refusal become user-visible safety defects. This milestone is complete when pinned config and observed installation produce structured `PreviewReport`, `ApplyPlan`, `PrunePlan`, and `StatusReport` values with tested edge cases.

### M4: Execution And Safe Mutation

Implement apply and prune execution against approved plans. The execution module owns private preflight, write ordering, last-mile filesystem safety, Managed Skill Content materialization, discovery symlink creation or update, lockfile writes, and Manifest updates.

Importance: execution is where policy becomes filesystem state, so it must revalidate before mutation. Apply ordering is a safety invariant: lockfiles, Managed Skill Content, discovery artifacts, then Manifest ownership. This milestone is complete when apply refuses blockers before mutation, records ownership last, supports forward recovery, and prune removes only manifest-owned stale state while skipping unsafe removals.
