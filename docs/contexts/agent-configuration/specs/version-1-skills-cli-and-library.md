...
- Because V1 implements Skill Configuration only, `config show` renders Skill Configuration, Default Client Selection, entry-level clients, Skill Configuration Entry Ids, Skill Sources, Skill Selection, Excluded Skills, and Skill Aliases without expanding all-skill selections from source contents.
...
- V1 persists Skill Configuration as inline `[[skills]]` rows in dotagents style: each row declares a **Skill Source** locator, optional **Git Source Ref**, optional **Skill Configuration Entry Id**, **Skill Selection**, and optional entry-level clients, Excluded Skills, and Skill Aliases.
- SelectSkill is exposed as `agentcfg skills select`. It does not accept client selection; new entries inherit the Config Layer's Default Client Selection. `skills select` fails before writing when the new or updated entry would not have a final client selection, and the diagnostic suggests `agentcfg clients set <client>...` before selecting Skills. `skills select` accepts an optional `--id` to set or target a **Skill Configuration Entry Id** on write.
- When selecting an explicit Included Skill, SelectSkill appends to an existing compatible Skill Configuration Entry instead of creating a duplicate entry. Compatibility requires the same **Skill Configuration Entry Id** when both entries declare one; otherwise the same Skill Source locator and **Git Source Ref** when present, final client selection, and no conflicting aliases or exclusions. `skills select` output reports the selected entry's inherited or explicit Clients, mentions `agentcfg skills clients ...` for client changes, and tells the User to run `agentcfg install` to materialize changes.
...
- Skill Configuration Entry mutations select an entry with `--id` when the entry declares a **Skill Configuration Entry Id**, or with `--source`, optional `--ref`, and `--skill` when needed. `--skill` matches Source Skill Name, not Skill Alias. `--id` and locator-based selectors must not disagree when both are supplied. Selectors that match zero or multiple entries fail and report enough matching entry detail for the User to disambiguate.
...
- Each Skill Configuration Entry may optionally declare a **Skill Configuration Entry Id** as `id`. Entry Ids must be unique within a Config Layer when declared.
...
- Test Skill Configuration Entry Id behavior: optional `id` on `[[skills]]` rows, unique Entry Ids within a Config Layer, `--id` selection for select/deselect and entry client mutations, locator fallback when no Entry Id is declared, and disagreement between `--id` and locator selectors fails validation.
